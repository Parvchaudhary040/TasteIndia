package com.parv.tasteindia.data.remote.dto

import kotlinx.serialization.SerialName
import kotlinx.serialization.Serializable

/**
 * Wire models for TheMealDB. These mirror the JSON exactly (flat `strIngredientN` fields,
 * everything a nullable string) and never leak past the data layer — the repository maps
 * them into [com.parv.tasteindia.domain.model] types.
 *
 * Every endpoint we use wraps its payload in `{"meals": [...] }`, and returns `{"meals": null}`
 * (HTTP 200) when nothing matches — hence the nullable list.
 */
@Serializable
...
data class MealDetailDto(
    ...
    /** Positional ingredient/measure pairs, slot 1..20, before any cleaning. */
